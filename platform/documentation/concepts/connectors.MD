...
| cacheMode |  Shall incoming data be compared to last data point and data only be ingested if differences based on hash codes or equality comparison are detected. | `CACHE_NONE` |
...
The basic wiring of a connector is:
* outInterface -> output -> app
* inInterface <- input <- app
...
The basic wiring of a channel connector is:
* outInterface -> outChannel(s) -> output -> app
* inInterface <- inChannel <- input <- app

While `JsonFormatter`/`JsonParser` do not require further information, `TextLineFormatter`/`TextLineParser` must be configured with a `separator`, e.g., "," for CSV, "\t" for TSV etc. If specified, `inSerializerClass` and `outSerializerClass` contribute to the pairing of `input`, `output`, `inInterface` and `outInterface`. Channel connectors must specify their input channel (`inChannel`) and their output channel (either `outChannel` or `outChannels`).

## OPC UA connector

The OPC UA  connector is a model connector with the following additional fields:

| Field | Description | Default |
| --- | --- | --- |
| browseNamePrefix |  Optional prefix string to be matched before the browse name of a UA node, undefined by default, may be set implicitly to `/` by the implementation. | `null` |

Regarding authentication, this connector supports `USERNAME`, `ISSUED`, `X509` and `ANONYMOUS` authentication tokens as well as TLS encryption.

`inInterface` or `outInterface` shall specify the `path` to an OPC node (_folder node, never a variable node_) where the fields/substructures of the respective `RecordType` are located. Currently, _a path never starts with a slash, it shall end with a slash and it always implicitly starts with `Root`_, i.e., a path like `/Root/Objects/PLCnext/Arp.Plc.Eclr/RobInstance01/` shown in some OPC UA tooling becomes `Objects/PLCnext/Arp.Plc.Eclr/RobInstance01/` in the model. The fields/substructures in the respective type must exactly match the browse names and data types as specified in the OPC server (the specification of NodeIds as we support it for IVML files derived from OPC specification XMLs may be made available here in one of the next versions of the platform.)

As the underlying implementation only provides rather coarse-grained (model-level) OPC Events, it is currently mandatory to specify a `samplingPeriod` to obtain data. Please also take into account that `Field` as a default caching time of `CACHE_ALWAYS` (static fields), i.e., for dynamically changing fields `CACHE_NONE` or a meaningful caching time for your application must be specified.

Optionally, the OPC UA connector allows to specify OPC UA NodeIds, using the specialized types.

A `NodeId` consist of

| Field | Description | Default |
| --- | --- | --- |
| identifier     | Numeric identifier of the OPC UA node id. | |
| nameSpaceIndex | Namespace index of the OPC UA node id. | |

A `OpcUaRecordType` refines `RecordType` for specifying the NodeId of the corresponding OPC UA node/folder. The additional fields are:

| Field | Description | Default |
| --- | --- | --- |
| nodeId | Optional NodeId. | | 

A `OpcUaField` refines `Field` for specifying the NodeId of a field, i.e., the corresponding OPC UA variable node. The additional fields are:

| Field | Description | Default |
| --- | --- | --- |
| nodeId | Optional NodeId. | | 

A `OpcUaEnumType` refines `EnumType` for specifying the NodeId of the enumeration type. The defined enumeration is for use within an oktoflow app and must match the respective OPC UA enumeration. The additional fields are:
...
This connector this connector supports `USERNAME`, `ISSUED`, `X509` and `ANONYMOUS` authentication tokens as well as TLS encryption.
...
Please note that only Influx v1 allows for an `USERNAME` token taken from the identity store. An authenticated `InfluxConnector` for Influx v2 expects an issued token, e.g.,

```
identities:
    "influxCloud": 
        type: ISSUED
        tokenData: my-hidden-token (usually 88 characters, ending with ==)
...
The `FileConnector` makes that in local files available or writes to local files, respectively. This is in particular helpful for initial testing of applications, but also required, in particular with CSV format in the energy metering domain. The `FileConnector` is a `ChannelConnector` and, thus, allows for serializers and parsers/formatters to be set up. Please note that the `FileConnector` is a generic type intended for platform extensions and so requires this setup, i.e., you have to know the respetive artifacts and classes. In contrast, there are pre-configured file connector types that already turn a generic `FileConnector` into, e.g., a JSON, a TSV or a CSV connector. Before directly relying on the `FileConnector` please refer first to the more specific file connector types.
...
The `MQTTv3` connector is a channel connector with no further fields. Channel names correspond to the channel/topic names on the MQTT broker, hierarchical names are separated by "/". 

Formatters and parsers can be applied as described for the `FileConnector`. 

This connector supports `USERNAME` authentication tokens and TLS encryption. If no authentication token is given, it implicitly applies anonymous authentication. Whether these forms of authentication are supported, in particular anonymous authentication, or wheter the right access control permissions are set so that the connector can access the configured data depends on the setup of the broker that you are addressing.

## MQTTv5 connector

The `MQTTv5` connector is a channel connector with no further fields. Channel names correspond to the channel/topic names on the MQTT broker, hierarchical names are separated by "/".
...
The `MQTT` connector is a channel connector which selects dynamically between `MQTTv3` and `MQTTv5` connector based on information provided by the underlying device, e.g., usually via an AAS. The MQTT connector has no further fields. Formatters and parsers can be applied as described for the `FileConnector`. This connector supports authentication and encryption as provided by the underlying connector. 
