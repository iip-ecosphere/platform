...
* Read log output, identify point where the service in question is loaded.
  * Can it be found? Is it really loaded by oktoflow/the app? Are there errors/issues to fix?
  * Does it fail in service initialization? Some logging output in your code may help to identify this part.
  * Does it process data? Temporary logging may help here (see also below the logging abilities of the platform).
  * Does it yet process data? If your service needs initialization, e.g., AI model loading, called in the wrong place may significantly delay the processing, in the extreme case the mock data is already ingested until your service is ready. 
    * Do not call longer-running initialization code in the service constructor rather than the service `start` method.
    * Delay the first mocking data item until your service is ready (`$period` entry). This may fail in future if you add further time-consuming services.
* If you seem to loose data, enable tracing on selected/all connectors/services, e.g., in IVML `traceSent=TraceKind::SYSOUT` (see [service configuration concepts](concepts/services.MD)). 
