    ...
    private static class FakeRtsaRestService<I, O> extends RtsaRestService<I, O> {
        ...
        protected String getClasspath(File rtsaPath) {
            ...
            if (!f.exists() || f.lastModified() < zip.lastModified()) {
                LoggerFactory.getLogger(getClass()).info("Unpacking {}", zip);
                ...
                try {
                    ...
                    LoggerFactory.getLogger(getClass()).error("Cannot unpack {}: {}", zip, e.getMessage());
                }
                File depl = new File("target/fake/myRtsaexample-0.1.0.zip");
                try {
                    Files.copy(depl, new File(new File(zipT, "deployments"), depl.getName()));
                } catch (IOException e) {
                    LoggerFactory.getLogger(getClass()).error("Cannot copy {}: {}", depl, e.getMessage());
                }
            }
