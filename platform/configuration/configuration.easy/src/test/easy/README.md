...
These models are partially in managed platform structure, partial in one simple file. Models in managed platform structure are intended for reuse in the testing/evaluation environment.
...
Service implementation stems from `test.configuration.configuration` implementing the services, based on generated interfaces, integrated via dynamic class loading; i.e., interfaces must be generated first, then the test implementation then the full apps.

`simpleMesh` is used for regression testing in `managementUI`.

More details in [summary.pdf](summary.pdf).