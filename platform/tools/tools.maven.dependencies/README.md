...
- `excludeTestArtifact` (default `false`, user property `mdep.excludeTestArtifact`) if `asTest` is active, enforce Maven `test` scope but still exclude/ignore the projects test artifact
