...
* For **Both**, in case of using multiple Python versions or multiple Python Virtual environments, the script will ask you to provide a runnable Python directory or a Python Virtual environment to set as ``IIP_PYTHON`` for the platform to use (You may skip this step). Also, You can pass an argument ``DepsOnly`` with the script to only install the platform dependencies without installing the full platform (In case you are developing an application without the need for the full platform). i.e. ``./fullInstallationLinux.sh DepsOnly`` for Linux, ``fullInstallationWin.bat DepsOnly`` for Windows.
