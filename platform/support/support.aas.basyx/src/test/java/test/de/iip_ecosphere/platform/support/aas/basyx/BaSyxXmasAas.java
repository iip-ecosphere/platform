...
public class BaSyxXmasAas extends XmasAas {
    ...
    public static void main(String[] args) {
        BaSyxXmasAas instance = new BaSyxXmasAas();
        instance.setCreateOperations(false); // fails in AASPackageExplorer (Dec 2023)
        instance.setCreateMultiLanguageProperties(false);
        if (args.length > 0) {
            if (args[0].equals("--withOperations")) {
                instance.setCreateOperations(true);
            }
        }
        try {
            instance.testCreateAndStore();
            System.out.println("Success: AASX package written to " + instance.getTargetFile());
        } catch (IOException e) {
