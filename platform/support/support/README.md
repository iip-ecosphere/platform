...
The container generation of the platform instantiation produces and installs an `installedDependencies.yml` by default. If you head for individual installations, please take care of a respective `installedDependencies.yml` file. The `installedDependencies.yml` may be located in the classpath, the actual directory (as default value for the folder determined by the `iip.installedDeps` system property) or the system root directory.

In addition to the settings above, an `installedDependencies.yml` may specify it's mode, i.e., `mode: TEST` for files that are just used in testing and `mode: PRODUCTION` (or omitted as this is the default) for production files. If the resolution including the classpath leads to a file in test mode, the implementation shall perform a second lookup without classpath resolution and, if there is another file, take that.
