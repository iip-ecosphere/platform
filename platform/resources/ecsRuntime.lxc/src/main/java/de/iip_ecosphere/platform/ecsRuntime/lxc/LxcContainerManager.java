...
public class LxcContainerManager extends AbstractContainerManager<LxcContainerDescriptor> {
    ...
    /**
     * Implements the factory descriptor for hooking the LXC container manager 
     ...
     * @param pathToYaml, The path to the directory where the YAML with container information is
     * @param lxcImageName, Gets the name of the base image of the container from the YAML
     * 		  if only the name is given it assumes to use local image
    ...
    public String addContainer(URI location) throws ExecutionException {
        ...
        try {
             ...
             * Second IF for container creation with remote image, should be removed if CURL can be corrected
             * 
             * **/
            if(lxcClient.loadContainer(containerName) == null && lxcImageFingerprint != null) {
            	...
            	LOGGER.info("Container with the same name already exists "
            			+ "or Remote Server cant be reached");
            }
     ...
     * If there is not running LXC daemon on the host it returns null.
     * 
     * @return LxcClient or <b>null</b> if no LXC daemon is running or if the lxc {@code lxcHost} in the 
     *     configuration setup cannot be applied, e.g., a Linux socket path
     */
    ...
    public void startContainer(String name) throws ExecutionException {
     ...
     // Um die States des Containers zu aktualisieren muessen passende LXC States hinzugefuegt werden
    ...
    public void publishImage(String containerName, String snapshot, String imageAlias, boolean isPublic) throws ExecutionException {
        ...
        try {
        	if(isPublic) {
        		lxcCliService.executeLinuxCmd("lxc publish " + containerName + "/" + snapshot + 
        										" --alias " + imageAlias + " --public");
        	} else {
        		lxcCliService.executeLinuxCmd("lxc publish " + containerName + "/" + snapshot + 
						" --alias " + imageAlias);
        	}
    ...
    /**
     * Copy/import an image from a remote server that is defined in {@coder RemoteServer.java} and set up on localhost
