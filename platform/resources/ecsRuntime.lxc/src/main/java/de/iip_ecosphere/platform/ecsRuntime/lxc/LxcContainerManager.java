...
public class LxcContainerManager extends AbstractContainerManager<LxcContainerDescriptor> {
    ...
    public String addContainer(URI location) throws ExecutionException {
        ...
        try {
            ...
            LxcSetup config = (LxcSetup) factory.getConfiguration();
            String downloadDirectory = config.getLxc().getDownloadDirectory();
            ...
            if (pathToYaml.endsWith("/")) {
                pathToYaml += config.getLxc().getLxcImageYamlFilename();
            }
            ...
            int port = 0;
            if (container.requiresPort(LxcContainerDescriptor.PORT_PLACEHOLDER)) {
            ...
            int port1 = 0;
            if (container.requiresPort(LxcContainerDescriptor.PORT_PLACEHOLDER_1)) {
                // may be gone until used, limit then netMgr ports in setup
                NetworkManager netMgr = NetworkManagerFactory.getInstance();
     ...
     */
    private String getImageName(LxcContainerDescriptor desc) {
     ...
     */
    private void configure(CreateContainerCmd cmd, int port, int port1, LxcContainerDescriptor  container) {
        ...
        if (container.getDood()) {
            ...
            String host = config.getLxc().getDockerHost();
    ...
    public DockerClient getDockerClient() {
        ...
        String dockerhost = config.getLxc().getDockerHost();
    ...
    public void startContainer(String id) throws ExecutionException {
        ...
        LxcContainerDescriptor container = getContainer(id, "id", "start");
    ...
    public void stopContainer(String id) throws ExecutionException {
        ...
        LxcContainerDescriptor container = getContainer(id, "id", "stop");
        String dockerId = container.getDockerId();
    ...
    public void undeployContainer(String id) throws ExecutionException {
        ...
        LxcContainerDescriptor container = getContainer(id, "id", "undeploy");
        String dockerId = container.getDockerId();
        ...
        FactoryDescriptor factory = new FactoryDescriptor();
        LxcSetup config = (LxcSetup) factory.getConfiguration();
        if (config.getLxc().getDeleteWhenUndeployed()) {
